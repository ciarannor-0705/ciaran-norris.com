...
export const Pagination = ({ next, previous, children }: PaginationProps) => {
  return (
    <nav aria-label="pagination" className="flex justify-between items-center gap-1">
      {previous && (
        <a href={previous.href} className="rounded-md py-1 pr-2 pl-7 !no-underline font-sans text-[--ds-gray-900] hover:text-[--ds-gray-1000] fill-[--ds-gray-900] hover:fill-[--ds-gray-1000] duration-150">
          <span className="text-[0.8125rem] leading-[1.125rem] mb-0.5">Previous</span>
          <div className="relative">
            <span className="text-[--ds-gray-1000] text-base font-medium">{previous.title}</span>
            <span className="absolute -left-[26px] mt-0.5"><ArrowLeftIcon /></span>
          </div>
        </a>
      )}
      {children && <div>{children}</div>}
      {next && (
        <a href={next.href} className="rounded-md ml-auto py-1 pl-2 pr-7 !no-underline font-sans text-[--ds-gray-900] hover:text-[--ds-gray-1000] fill-[--ds-gray-900] hover:fill-[--ds-gray-1000] duration-150">
          <span className="text-[0.8125rem] leading-[1.125rem] mb-0.5">Next</span>
          <div className="relative">
            <span className="text-[--ds-gray-1000] text-base font-medium">{next.title}</span>
            <span className="absolute -right-[26px] mt-0.5"><ArrowRightIcon /></span>
          </div>
        </a>
      )}
    </nav>
