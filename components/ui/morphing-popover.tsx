...
import {
  ...
  createContext, useContext, isValidElement,
} from 'react';
...
function MorphingPopoverTrigger({ children, className, ...props }: MorphingPopoverTriggerProps) {
  ...
  return (
    <motion.div key={context.uniqueId} layoutId={`popover-trigger-${context.uniqueId}`} onClick={context.open}>
      <motion.button {...props} layoutId={`popover-label-${context.uniqueId}`} key={context.uniqueId} className={className} aria-expanded={context.isOpen}>
        {children}
      </motion.button>
    </motion.div>
  );
...
function MorphingPopoverContent({ children, className, ...props }: MorphingPopoverContentProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => { if (e.key === 'Escape') context.close(); };
    ...
    <AnimatePresence>
      {context.isOpen && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          className={cn('absolute overflow-hidden rounded-xl border border-black/10 bg-white p-5 shadow-lg z-50', className)}
          initial="initial"
          animate="animate"
          exit="exit"
        ...
        >
          <button
            ...
            className="absolute top-3 right-3 text-black/40 hover:text-black transition-colors text-sm"
            aria-label="Close"
          >
            ✕
          </button>
